...
int main()
{
	...
	while (true)
	{
		...
		if (FD_ISSET(serverSocket, &readable))
		{
			...
			if (conns.size() >= CLIENT_LIMIT){
				send(clientSocket, "Max Clients Connected closing connection", 0, 0);
				printf("Client tried to connect, but max clients reached - closing connection...");
		...
		for (auto it = conns.begin(); it != conns.end(); )  // note no ++it here
		{
			...
			//is there data to write to this 
